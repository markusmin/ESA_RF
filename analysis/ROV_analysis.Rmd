...
variance calculated as the product of surface area and the variance of
...
HC_PS_15_16_high_stratum_YE_variance <- HC_PS_15_16_high_surface_area_m2*HC_PS_15_16_mean_high_stratum_density_var
```
...
# Subset Hood Canal transects
nonHC_PS_15_16_transects <- subset(PS_15_16_transects, DPS == "Hood Canal")
...
Total abundance ($P$) in numbers of individuals was the product of thestratum surface area ($SA_s$) and the mean taxon density ($D$), with variance calculated as the product of surface area and the variance of mean stratum density: 
...
nonHC_PS_15_16_high_stratum_YE_variance <- nonHC_PS_15_16_high_surface_area_m2*nonHC_PS_15_16_mean_high_stratum_density_var
```
...
read_excel(paste0(ROV_data_path, "/2018 SJI area swept estimates_17May2021.xlsx"), skip = 3)[,c(1:4)] %>% 
...
# For now, drop the two transects (TO BE UPDATED) in area swept and species files
SJI_2018_area_swept <- subset(SJI_2018_area_swept, !(Tran_ID %in% c("H014", "H029")))
SJI_2018_species <- subset(SJI_2018_species, !(Tran_ID %in% c("H014", "H029")))
...
SJI_2018_high_stratum_YE_variance <- SJI_2018_high_surface_area_m2*SJI_2018_mean_high_stratum_density_var
```
...
survey_names <- c("2008 San Juan Islands", "2010 San Juan Islands", "2015-16 Puget Sound", "2015-16 Puget Sound", "2018 San Juan Islands", "2018 Gulf Islands", "2018 Vector")

survey_areas <- c("San Juan Islands", "San Juan Islands", "Puget Sound Proper (excluding Hood Canal)", "Hood Canal", "San Juan Islands", "Gulf Islands (CA)", "Strait of Georgia (CA)")

years <- c("2008", "2010", "2015, 2016", "2015, 2016", "2018", "2018", "2018")


YE_ROV_results_summary <- data.frame(survey = survey_names, area = survey_areas, year = years, species = rep("Yelloweye Rockfish", length(survey_names)), abundance = rep(0,length(survey_names)), CV = rep(0,length(survey_names)))
