...
RF_ratios_70_87_comb_plot <- ggplot(RF_ratios_70_87_comb, aes(x = gear, y = prop, fill = species))+
  ...
  geom_text(data = RF_ratios_70_87_comb[!duplicated(RF_ratios_70_87_comb[,c("gear")]),], aes(x = gear, y = 1.03, label = total_catch_by_gear), hjust = 0)+
...
```{r load_yellowbook_data}
BV_path <- here("catch_reconstruction_data", "commercial", "BoundVol_1935_1940_RkFsh.xlsx")

BV_1935 <- read_excel(BV_path, sheet = "1935")

BV_1936_1940 <- read_excel(BV_path, sheet = "1936, 1937-1940")

# Combine two dfs
setdiff(colnames(BV_1935), colnames(BV_1936_1940))
setdiff(colnames(BV_1936_1940), colnames(BV_1935))

# Drop the columns that aren't in both (these aren't informative)

BV_1935 %>% 
  dplyr::select(-c(Utilization)) -> BV_1935

BV_1936_1940 %>% 
  dplyr::select(-c(XRefCurrDispCode_Fkey, Select4Edit)) -> BV_1936_1940

BV_1935 %>% 
  bind_rows(., BV_1936_1940) -> BV_35_40
```
...
YE_commercial_catch_plot <- ggplot(YE_catch_estimates_1921_2020, aes(x = year, y = yelloweye_catch_estimate, fill = source))+
  geom_bar(stat = "identity")+
  scale_fill_tableau(palette = "Tableau 10")+
  ggtitle("Historical Yelloweye Catch")+
  ylab("Catch (pounds)") + 
  xlab("Year")+
