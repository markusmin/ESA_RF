...
Here's another big problem: The data that I have is currently summed across "North Puget Sound" and "South Puget Sound". Hood Canal is included in "South Puget Sound". I'm sure this was summed at some point from finer resolution data, but can we find that data?
...
rec_NPS %>% 
  ...
  dplyr::select(Year, `Rockfish - General`) -> rec_NPS_94_02
...
rec_NPS_94_02 %>% 
  ...
  mutate(., high_catch_n = total*ye_prop*3) %>%
  mutate(., medium_catch_n = total*ye_prop*2) %>%
  mutate(., low_catch_n = total*ye_prop*1) %>%
  mutate(., high_catch = high_catch_n*YE_lbs_per_individual) %>% 
  mutate(., medium_catch = medium_catch_n*YE_lbs_per_individual) %>% 
  mutate(., low_catch = low_catch_n*YE_lbs_per_individual) %>% 
...
rec_NPS_94_02 %>% 
  dplyr::rename(year = Year, total = `Rockfish - General`) %>%
  mutate(., high_catch_n = total*boc_prop*3) %>%
  mutate(., medium_catch_n = total*boc_prop*2) %>%
  mutate(., low_catch_n = total*boc_prop*1) %>%
  mutate(., high_catch = high_catch_n*BOC_lbs_per_individual) %>% 
  mutate(., medium_catch = medium_catch_n*BOC_lbs_per_individual) %>% 
  mutate(., low_catch = low_catch_n*BOC_lbs_per_individual) %>% 
...
rec_SPS %>% 
  subset(., Year >= 1994 & Year <= 2002) %>% 
  dplyr::select(Year, `Rockfish - General`) -> rec_SPS_94_02
