...
buck65_annual_sums %>% 
  mutate(boc_prop = ifelse(PS == "North Puget Sound", subset(rec_mean_spp_comp_NPS, Species == "Bocaccio")$`1965-1967a`/100, subset(rec_mean_spp_comp_SPS, Species == "Bocaccio")$`1965-1967a`/100)) %>% 
  mutate(ye_prop = ifelse(PS == "North Puget Sound", subset(rec_mean_spp_comp_NPS, Species == "Yelloweye")$`1965-1967a`/100, subset(rec_mean_spp_comp_SPS, Species == "Yelloweye")$`1965-1967a`/100)) -> buck65_annual_sums

# U.S. DPS minus Hood canal
buck65_annual_sums %>% 
  subset(Species %in% c("Rockfishes", "Yelloweye rockfish")) %>% 
  mutate(., catch = ifelse(Species == "Yelloweye rockfish", total_catch, ye_prop*total_catch)) %>% 
  subset(PS != "Hood Canal") %>% 
  ungroup() %>% 
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1965) %>% 
  mutate(catch_lbs = catch*YE_lbs_per_individual) -> ye_catch_buck65_USDPS
...
buck65_annual_sums %>% 
  ...
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1965) %>% 
  mutate(catch_lbs = catch*YE_lbs_per_individual) -> ye_catch_buck65_HC

# Prorate and summarise for bocaccio
buck65_annual_sums %>% 
  subset(Species %in% c("Rockfishes", "Bocaccio")) %>% 
  mutate(., catch = ifelse(Species == "Bocaccio", total_catch, boc_prop*total_catch)) %>% 
  ungroup() %>% 
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1965) %>% 
  mutate(catch_lbs = catch*BOC_lbs_per_individual) -> boc_catch_buck65
...
buck66_annual_sums %>% 
  mutate(boc_prop = ifelse(PS == "North Puget Sound", subset(rec_mean_spp_comp_NPS, Species == "Bocaccio")$`1965-1967a`/100, subset(rec_mean_spp_comp_SPS, Species == "Bocaccio")$`1965-1967a`/100)) %>% 
  mutate(ye_prop = ifelse(PS == "North Puget Sound", subset(rec_mean_spp_comp_NPS, Species == "Yelloweye")$`1965-1967a`/100, subset(rec_mean_spp_comp_SPS, Species == "Yelloweye")$`1965-1967a`/100)) -> buck66_annual_sums
...
buck66_annual_sums %>% 
  ...
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1966) %>% 
  mutate(catch_lbs = catch*YE_lbs_per_individual) -> ye_catch_buck66_USDPS
...
buck66_annual_sums %>% 
  ...
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1966) %>% 
  mutate(catch_lbs = catch*YE_lbs_per_individual) -> ye_catch_buck66_HC
...
buck66_annual_sums %>% 
  ...
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1966) %>% 
  mutate(catch_lbs = catch*BOC_lbs_per_individual) -> boc_catch_buck66
...
buck67_annual_sums %>% 
  mutate(boc_prop = ifelse(PS == "North Puget Sound", subset(rec_mean_spp_comp_NPS, Species == "Bocaccio")$`1965-1967a`/100, subset(rec_mean_spp_comp_SPS, Species == "Bocaccio")$`1965-1967a`/100)) %>% 
  mutate(ye_prop = ifelse(PS == "North Puget Sound", subset(rec_mean_spp_comp_NPS, Species == "Yelloweye")$`1965-1967a`/100, subset(rec_mean_spp_comp_SPS, Species == "Yelloweye")$`1965-1967a`/100)) -> buck67_annual_sums
...
buck67_annual_sums %>% 
  ...
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1967) %>% 
  mutate(catch_lbs = catch*YE_lbs_per_individual) -> ye_catch_buck67_USDPS
...
buck67_annual_sums %>% 
  ...
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1967) %>% 
  mutate(catch_lbs = catch*YE_lbs_per_individual) -> ye_catch_buck67_HC
...
buck67_annual_sums %>% 
  ...
  summarise(catch = sum(catch)) %>% 
  mutate(year = 1967) %>% 
  mutate(catch_lbs = catch*BOC_lbs_per_individual) -> boc_catch_buck67
...
ye_catch_bargmann_HC %>% 
  ...
  bind_rows(., ye_catch_buck65_HC) -> ye_catch_65_69_HC

ye_catch_bargmann_USDPS %>% 
  mutate(year = as.numeric(year)) %>% 
  bind_rows(., ye_catch_buck67_USDPS) %>% 
  bind_rows(., ye_catch_buck66_USDPS) %>% 
  bind_rows(., ye_catch_buck65_USDPS) -> ye_catch_65_69_USDPS

boc_catch_bargmann %>% 
  mutate(year = as.numeric(year)) %>% 
  bind_rows(., boc_catch_buck67) %>% 
  bind_rows(., boc_catch_buck66) %>% 
  bind_rows(., boc_catch_buck65) -> boc_catch_65_69
...
WDFW_38_41_path <- here("catch_reconstruction_data", "recreational", "pre_1970", "1938_1941_sportcatch.xlsx")

read_excel(WDFW_38_41_path, sheet = "1938") %>% 
  mutate(year = 1938) %>% 
  mutate(red_snapper = as.numeric(red_snapper)) -> rec_38
read_excel(WDFW_38_41_path, sheet = "1939") %>% 
  mutate(year = 1939) %>% 
  mutate(red_snapper = as.numeric(red_snapper)) -> rec_39
read_excel(WDFW_38_41_path, sheet = "1940") %>% 
  mutate(year = 1940) -> rec_40
read_excel(WDFW_38_41_path, sheet = "1941") %>% 
...
ggplot(rec_38_41, aes(x = factor(month, levels = c("February", "March", "April", "May", "June", "July", "August", "September", "October", "November")), y = rock_cod/no_boathouses, color = factor(year)))+
  geom_point()
